...
exports.handler = async (event) => {
  try {
    ...
    if (cached && Date.now() - cached.time < TTL) {
      return { statusCode: 200, headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'public, max-age=60, s-maxage=120', 'x-fotboltavaktin-cache': 'hit' }, body: JSON.stringify(cached.body) };
    }
    ...
    const [table, report] = await Promise.all([
      getBestCompetitionTable(matches, match),
      getMatchReport(match)
    ]);
    ...
    return {
      statusCode: 200,
      headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'public, max-age=60, s-maxage=120', 'x-fotboltavaktin-cache': 'miss' },
